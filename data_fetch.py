...
def get_team_xwoba(season: int, start_dt: Optional[str] = None, end_dt: Optional[str] = None) -> dict:
    """Team-level xwOBA. There is no direct team-level expected-stats function
    in pybaseball -- it only exposes player-level statcast_batter_expected_stats
    and statcast_pitcher_expected_stats (confirmed). A team aggregate could be
    built by pulling all batters' expected stats for the season and weighting
    by plate appearances per team, but that adds real complexity/fragility
    (needs a separate team-roster mapping, handles mid-season trades, etc.)
    for a value that's a "nice to have" cross-check, not a hard requirement --
    the model runs fine on the actual-scoring-rate model as primary when this
    returns {}. Left as a known gap rather than a guessed, unverified
    implementation; happy to build the aggregation if you want the xwOBA
