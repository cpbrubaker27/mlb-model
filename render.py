...
def render_game_card(meta: dict, home_inputs, away_inputs, park_weather, market, prediction) -> str:
  ...
  <div class="card" id="{game_id}" data-best-tier="{prediction.best_tier}">
    ...
    <div class="section">
      ...
      <div class="row"><span class="pick-label">{prediction.ml_pick}</span>{tier_pill(prediction.ml_tier)}</div>
      <div class="small">Model <span class="num">{prediction.model_home_win_pct}%</span> vs Market <span class="num">{prediction.market_home_win_pct}%</span> (home) &middot; edge <span class="num">{prediction.ml_edge_pts:+.1f}</span> pts</div>
      {ml_notes_html}
    </div>
    <div class="section">
