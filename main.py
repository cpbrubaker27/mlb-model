...
def main():
    ...
    try:
        season_batting = df.get_team_batting(SEASON)
        trailing_batting = df.get_team_batting(SEASON, trailing_start, trailing_end)
        season_pitching = df.get_team_pitching(SEASON)
        trailing_pitching = df.get_team_pitching(SEASON, trailing_start, trailing_end)
    except Exception as e:
        print(f"FATAL: could not fetch FanGraphs season/trailing stats: {e}")
        sys.exit(1)
